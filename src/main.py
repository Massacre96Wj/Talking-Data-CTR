...
def cal_cvr(train, test, type='float32'):
    ...
    for i, (train_index, test_index) in enumerate(kf):
        ...
        tr['cvr_gb_ip_day_hour'] = GaussianSmoth().update_moment(tr['count_gb_ip_day_hour'],
                                                                 tr['sum_label_gb_ip_day_hour'])
        tr['cvr_gb_ip_app'] = GaussianSmoth().update_moment(tr['count_gb_ip_app'],
                                                            tr['sum_label_gb_ip_app'])
        tr['cvr_gb_ip_app_os'] = GaussianSmoth().update_moment(tr['count_gb_ip_app_os'],
                                                               tr['sum_label_gb_ip_app_os'])
...
def lgb_cv(train_feature, train_label, test_feature, test_label, params, folds, rounds):
    ...
    res = lgb.train(params, dtrain, num_round, valid_sets=[dtest], valid_names=['test'],
...
train = pd.read_csv(root_path + 'data/input/train.csv', header=0, sep=',', dtype=dtypes,
                    usecols=['ip', 'app', 'device', 'os', 'channel', 'click_time', 'is_attributed'])
test_supplement = pd.read_csv(root_path + 'data/input/test_supplement.csv', header=0, sep=',', dtype=dtypes,
...
log('Read test...')
test = pd.read_csv(root_path + 'data/input/test.csv', header=0, sep=',', dtype=dtypes,
                   usecols=['click_id', 'ip', 'app', 'device', 'os', 'channel', 'click_time'],
                   parse_dates=['click_time'])
log('Test data original shape: ' + str(test.shape))

test = test.merge(test_supplement.drop_duplicates(subset=['ip', 'app', 'device', 'os', 'channel', 'date'],
                keep='first'), left_on=['ip', 'app', 'device', 'os', 'channel', 'click_time'],
                right_on=['ip', 'app', 'device', 'os', 'channel', 'date'], how='left')
...
local_train_x, local_train_y, local_test_x, local_test_y = get_model_input_data(local_train, local_test,
                                                                                is_local=1)
del local_train
del local_test
gc.collect()
...
iterations_lgb, best_score_lgb, model_cv_lgb = lgb_cv(local_train_x, local_train_y, local_test_x,
                                                      local_test_y, params_lgb, config_lgb['folds'],
                                                      config_lgb['rounds'])
...
importance_lgb = sorted(zip(online_train_x.columns, model_cv_lgb.feature_importance("gain")),
...
res_lgb = store_result(pd.read_csv(root_path + 'data/input/test.csv', header=0, sep=',',
                        usecols=['click_id']).click_id.astype(int), pred_lgb,
                       '20180507-lgb-%f(r%d)' % (best_score_lgb, iterations_lgb))
